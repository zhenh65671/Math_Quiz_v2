...
class Quiz:
    ...
    def check_answer(self):
        ...
        print("getting num correct", num_correct)
        ...
        # Get question and answer for rounds / checking
        var_correct = self.correct.get()
        var_question = self.question_asked.get()
        ...
        try:
            ...
            if gk_ans != correct:
                ...
                self.submit_button.config(state=DISABLED)
                round_results = "{} = {}, <incorrect, answer was {}>".format(var_question, gk_ans, var_correct)
                num_complete += 1
                num_wrong += 1
                ...
                self.next_button.config(state=NORMAL)
                self.submit_button.config(state=DISABLED)
                round_results = "{} = {}, <correct>".format(var_question, gk_ans)
                num_correct += 1
                print("added one to num correct", num_correct)
                num_complete += 1
                self.round_results_list.append(round_results)
                self.quiz_round_result[1] = num_correct

            print("results", self.quiz_round_result)
            ...
            self.answer_error_label = Label(self.questions_frame, text="Please enter you answer",
                                            font="Arial 10 bold", fg="black", bg="cyan", pady=10, width=40)
    ...
    def next_question(self):
        ...
        # set question (for rounds history)
...
class Export:
    def __init__(self, partner, quiz_results, round_results, ):
        ...
        self.save_button = Button(self.save_cancel_frame, text="Save",
                                  font="Arial 15 bold", bg="#003366", fg="white",
                                  command=lambda: self.save_history(partner, round_results,  quiz_results))
    ...
    def save_history(self, partner, num_questions, round_results):

        # get the number of questions that user has answer
        print("num question", num_questions)
        num_ans = num_questions
        ...
        if has_error == "yes":
            ...
            # Quiz stats
            f.write("Correct Answers : {} \n".format(num_questions[1]))
            f.write("Incorrect Answers : {} \n".format(int(num_questions[0]) - int(num_questions[1])))
            f.write("questions Answered: {} \n".format(num_questions[0]))
            f.write("round results: {} \n".format(round_results))
            f.close()
